...
// exit();
// Variables
$email_subject = '[Reminder] Outstanding Uang Muka';
$email_body = '
<p>
    Kepada Yth Bapak/Ibu #TITLE# #NAME#
</p>
<p>
    Bersama ini kami informasikan bahwa hingga saat ini Bapak/ Ibu masih belum mempertanggungjawabkan uang muka sebesar <b>Rp #G_TOTAL#,-</b> , dengan rincian sebagai berikut :
</p>
<p>
    <table border="1">
        <tr>
...
<p>
    Sehubungan dengan hal tersebut di atas, kami mohon bantuan dari Bapak/Ibu untuk dapat <u><b>segera</b></u> menyerahkan Laporan Pertanggungjawaban uang muka tersebut ke Bagian Keuangan - #PIC#. Apabila laporan pertanggungjawaban sudah pernah diserahkan, mohon mengonfirmasi kembali ke Bagian Keuangan.
</p>
<p>
    Sebagai informasi, uang muka yang belum dipertanggungjawabkan akan mempengaruhi persetujuan pengajuan uang muka berikutnya.
</p>
<p>
...
foreach ($email_to as $key => $value) {
    ...
    $sql1 = "SELECT * FROM blast_email_um5 WHERE `email_send` = 'N' AND email = '".$email_to[$key]['email']."' LIMIT 1000;";
    // print_r($sql1);
    $result1 = $conn->query($sql1);
    $tbl_row = "";
    ...
    // Sending email
    sendEmail ($email_to[$key]['email'], $email_to[$key]['nik'], $email_to[$key]['name'], $email_to[$key]['title'], $email_to[$key]['pic'], $email_to[$key]['pic_email'], $email_to[$key]['atasan'], $email_to[$key]['atasan_email'], $email_to[$key]['tbl_row'], $email_to[$key]['g_total'], $email_to[$key]['send_id'], $email_subject, $email_body, $email_body_alt);
...
function sendEmail ($email_to, $nik, $name, $title, $pic, $pic_email, $atasan, $atasan_email, $tbl_row, $g_total, $send_id, $email_subject, $email_body, $email_body_alt = '')
{
    ...
    try {
        ...
        // CC and BCC
        // if ($atasan_email != '' && isset($atasan_email) && !empty($atasan_email)) {
        //     $mail->addCC($atasan_email);
        // }
        // $mail->addCC($atasan_email);                                // Add CC #1
        ...
        foreach ($pieces as $key2 => $value2) {
            if ($value2 != "" && !empty($value2)) {
                $sql3 = "UPDATE blast_email_um5 SET email_send='Y', email_sendDtm='$now' WHERE id=$value2";
                // print_r($key2."-".$value2);                
