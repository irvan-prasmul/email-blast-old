...
//     $sql1 = "SELECT * FROM blast_email_um_thx WHERE `email_send` = 'N' AND nik = '".$data[$key]['nik']."' LIMIT 1000;";
//     // print_r($sql1);
...
// exit();
// Variables
$email_subject = '(KONFIRMASI) Laporan Pertanggungjawaban Uang Muka';
$email_body = '
<p>
    Kepada Yth #TITLE# #NAME#
</p>
<p>
    Bersama ini kami mengonfirmasikan bahwa Laporan Pertanggungjawaban Uang Muka yang telah #TITLE# kirimkan telah kami terima dengan rincian sebagai berikut :
</p>
<p>
    <table border="1">
        <tr>
...
<p>
    Sehubungan dengan hal tersebut di atas, kami mengucapkan terima kasih untuk konfirmasi dan kerjasamanya sehingga Laporan Pertanggungjawaban Uang Muka dapat diselesaikan.
</p>
<p>
...
foreach ($email_to as $key => $value) {
    ...
    $sql1 = "SELECT * FROM blast_email_um_thx WHERE `email_send` = 'N' AND email = '".$email_to[$key]['email']."' LIMIT 1000;";
    // print_r($sql1);
    $result1 = $conn->query($sql1);
    ...
    try {
        ...
        foreach ($pieces as $key2 => $value2) {
            if ($value2 != "" && !empty($value2)) {
                $sql3 = "UPDATE blast_email_um_thx SET email_send='Y', email_sendDtm='$now' WHERE id=$value2";
                // print_r($key2."-".$value2);                
